...
int gettimeofday(struct timeval* tp, struct timezone* tzp)
{
	...
	SYSTEMTIME  system_time;//#include <time.h>
#include <vector>
...
#include <windows.h>
	...
	void sortstring() {
		...
		makestringarrays(array, arraysize, numarrays, 0, slength, 4);
	...
	void sortint() {
		...
		makeintarrays(array, arraysize, numarrays, 0.8, 4);
	...
	void benchmarkstring(char*** arrays, std::vector<char*>*v, int arraysize, int numarrays, float sortedness, int slength) {
		struct timeval stop, start;
		double seconds;
		...
		printf("timsort - arraysize = %d, number of arrays = %d, degree sorted %d%% - time %f\n", arraysize, numarrays, (int)(sortedness * 100), seconds);
		...
		gettimeofday(&stop, NULL);
		seconds = (double)(stop.tv_sec - start.tv_sec) + (double)(stop.tv_usec - start.tv_usec) / 1000000;
		printf("pdqsort - arraysize = %d, number of arrays = %d, degree sorted %d%% - time %f\n", arraysize, numarrays, (int)(sortedness * 100), seconds);
		...
		gettimeofday(&stop, NULL);
		seconds = (double)(stop.tv_sec - start.tv_sec) + (double)(stop.tv_usec - start.tv_usec) / 1000000;
		printf("tqsort - arraysize = %d, number of arrays = %d, degree sorted %d%% - time %f\n\n", arraysize, numarrays, (int)(sortedness * 100), seconds);
	}


	void benchmarkint(int** arrays, std::vector<int>*v, int arraysize, int numarrays, float sortedness) {
		struct timeval stop, start;
		double seconds;
	...
	void benchmark() {
		...
		sortedness = 0;
		benchmarkint(arrays, v, arraysize, numarrays, sortedness);
		...
		sortedness = 0;
		benchmarkstring(arraystring, vstring, arraysize, numarrays, sortedness, slength);
...
void sortstring() {
	...
	//printf("comparecount %d \n", globalcomparecount);
	for (int i = 0; i <1 ; i++) {//numarrays
		if (check3<char*>((char**)&v[i][0], array[i], arraysize, numarrays, std::less<char*>())) {
...
void sortint() {
	...
	printf("comparecount %d \n", globalcomparecount);
	//check2<int>(array, m, p, compareint);
	if (check3<int>((int*)&v[0][0],array[0], arraysize, 1, compareint)) {
		printf("\nsuccess\n");
	}
	else {
		printf("\nfail\n");
